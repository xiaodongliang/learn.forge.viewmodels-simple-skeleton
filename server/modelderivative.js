...
router.post('/api/forge/modelderivative/jobs', jsonParser, function (req, res) {
    oauth.getTokenInternal().then(function (credentials) {
        ...
        postJob.input.urn = req.body.objectKey.toBase64();
        ...
        derivativesApi.translate(postJob, {}, oauth.OAuthClient(), credentials)
            .then(function (data) {
                res.status(200).json({base64urn: postJob.input.urn});
            }).catch(function (e) {
...
router.get('/api/forge/modelderivative/status', function (req, res) {
    ...
    oauth.getTokenInternal().then(function (credentials) { 
        ...
        var derivativesApi = new forgeSDK.DerivativesApi();
        // post the job
        derivativesApi.getManifest(base64urn, {}, oauth.OAuthClient(), credentials)
            .then(function (data) {
                res.status(200).json(data.body);
            }).catch(function (e) {
                console.log('Error at Model Derivative manifest:');
                console.log(e);
                res.status(500).json({ error: e.error.body })
            });
    }); 
